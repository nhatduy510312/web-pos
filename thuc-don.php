...
<?php foreach ($section['items'] as $item): ?><article class="menu-item" data-menu-item data-search="<?= ghe_h($section['title'] . ' ' . $item['name'] . ' ' . $item['detail']) ?>"><?php if(!empty($content['media'][$item['image']])) ghe_photo($item['image'],'menu-photo'); ?><div class="menu-item-top"><h3><?= ghe_h($item['name']) ?></h3><span><?= ghe_price($item['price']) ?></span></div><p><?= ghe_h($item['detail']) ?></p><?php if ($item['tags']): ?><div class="tags"><?php foreach ($item['tags'] as $tag): ?><span><?= ghe_h($tag) ?></span><?php endforeach; ?></div><?php endif; ?></article><?php endforeach; ?>
</div></section>
<?php endforeach; ?>
